...
namespace Com.LuisPedroFonseca.ProCamera2D
{
    ...
    public class ProCamera2D : MonoBehaviour, ISerializationCallbackReceiver
    {
        ...
        public void UpdateScreenSizeReturn(float newSize, bool checkReturnZoom, float duration = 0f, EaseType easeType = EaseType.EaseInOut)
        {
            if (!enabled)
                return;
            ...
            if (duration > 0)
                _updateScreenSizeCoroutine = StartCoroutine(UpdateScreenSizeRoutine(newSize, duration, easeType, checkReturnZoom));
            else
                SetScreenSize(newSize);
        }
        ...
        /// <param name="easeType">The easing method to apply. Only used when the duration is bigger than 0</param>
        public void Zoom(float zoomAmount, float duration = 0f, EaseType easeType = EaseType.EaseInOut, bool checkReturn = true)
        {
            UpdateScreenSizeReturn(ScreenSizeInWorldCoordinates.y * .5f + zoomAmount,checkReturn, duration, easeType);
        }
        ...
        IEnumerator UpdateScreenSizeRoutine(float finalSize, float duration, EaseType easeType, bool checkReturn = true)
        {
            ...
            if (OnUpdateScreenSizeFinished != null && checkReturn)
